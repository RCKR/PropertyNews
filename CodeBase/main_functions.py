...
def content_text(article_: bs.Tag, atr: str, desc: str = None, id_: str = None) -> str or None:
...
def get_id_from_main_page(article_: bs.Tag):
...
def get_article_id(article_: bs.Tag):
    article_id = ''
    try:
...
def check_if_article_is_premium(article_: bs.Tag) -> bool:
    if article_.find('article', class_='entry locked-paywalled__item'):
        return True
    return False

def get_single_article_data(article_: bs.Tag) -> dict:
...
def find_articles_on_page(soup: bs.BeautifulSoup) -> bs.ResultSet:
...
def run_single_page_pipe(soup: bs.BeautifulSoup) -> list:
    articles = find_articles_on_page(soup)
